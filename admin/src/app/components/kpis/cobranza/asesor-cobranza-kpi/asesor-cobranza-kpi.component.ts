...
@Component({
  selector: 'app-asesor-cobranza-kpi',
  templateUrl: './asesor-cobranza-kpi.component.html',
  styleUrls: ['./asesor-cobranza-kpi.component.css']
})
export class AsesorCobranzaKpiComponent implements OnInit {
  ...
  init_data(){
    this.load_btn = true;
    this._kpiService.get_pagos_agente(this.inicio,this.hasta,this.token).subscribe(
        response=>{
            console.log(response);
            this.arr_colaboradores = response.general;
            this.init_chart_general();
            if(response.data != undefined){
              for(var item of response.data){
                //SEM1
                for(var sem of item.sem_uno){
                  for(var pago of item.pagos){
                      if(sem._id.toString() == pago.colaborador.toString()){
                          if(pago.day >= 1 && pago.day <= 7){
                              sem.total = sem.total + parseFloat(pago.monto);
                          }
                      }
                  }
                }
  ...
  init_chart_general(){
    setTimeout(() => {
        ...
        let arr_colaboradores = [];
        let arr_montos = [];

        for(var item of this.arr_colaboradores){
          if(item.total > 0){
            arr_colaboradores.push(item.nombres + ' ' + item.apellidos);
