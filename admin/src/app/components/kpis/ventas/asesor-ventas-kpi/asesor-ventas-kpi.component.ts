...
@Component({
  selector: 'app-asesor-ventas-kpi',
  templateUrl: './asesor-ventas-kpi.component.html',
  styleUrls: ['./asesor-ventas-kpi.component.css']
})
export class AsesorVentasKpiComponent implements OnInit {
  ...
  init_data(){
    this.load_btn = true;
    this._kpiService.get_ventas_agente(this.inicio,this.hasta,this.token).subscribe(
        response=>{
              ...
              for(var item of response.data){
                ...
                for(var sem of item.sem_uno){
                  for(var venta of item.ventas){
                      if(sem._id.toString() == venta.colaborador.toString()){
