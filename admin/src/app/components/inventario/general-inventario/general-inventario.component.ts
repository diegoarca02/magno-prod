...
export class GeneralInventarioComponent implements OnInit {
  ...
  constructor(
    ...
    for(var item of this.parameters){
      if(item.permiso == 'inventario_cuatro'){
  ...
  ngOnInit(): void {
    if(this.permisos.inventario_cuatro){
      this.init_almacenes();
    }else{
  ...
  filtrar(){
    if(!this.filtro_periodo){
      toastr.error("Periodo requerido.");
    }else{
      this.load_rollos = true;
      this._pedidoService.rollos_general(this.filtro_periodo,this.token).subscribe(
        response=>{
          this.rollos = [];
